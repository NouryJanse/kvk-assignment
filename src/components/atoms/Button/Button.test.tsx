...
it('renders with type reset', async () => {
  render(
    <Button type="reset" label="test button" disabled={false} fullwidth={false} noedge={false} buttonStyle="primary" />,
  )
  const ButtonComponent = screen.getByRole('button')
  expect(ButtonComponent).toBeInTheDocument()
...
it('renders with type submit and a disabled property', async () => {
  render(
    <Button type="submit" label="test button" disabled={true} fullwidth={false} noedge={false} buttonStyle="primary" />,
  )
  const ButtonComponent = screen.getByRole('button')
  expect(ButtonComponent).toBeInTheDocument()
...
it('renders with a child component', async () => {
  ...
  const { queryByTestId } = render(
    <Button type="submit" disabled={false} fullwidth={false} noedge={false} buttonStyle="primary">
      {child}
    </Button>,
  )
